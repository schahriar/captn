...
// TestRustGraphDump prints the observation graph for a Rust search so node
...
func TestRustGraphDump(t *testing.T) {
	...
	// rust-analyzer answers definitions empty until cargo metadata loads
	var og *cog.ObservationGraph

	for attempt := 0; attempt < 10; attempt++ {
		g, _, err := wspace.SearchSnippet(t.Context(), "./fixtures/rust/multidep/src/main.rs", "seen.insert(example_text(), 1)")
