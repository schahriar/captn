...
const (
	htmlServerName              = "vscode-html-language-server"
	vscodeServersInstallCommand = "npm install -g vscode-langservers-extracted"
)

func htmlServerPath(ctx context.Context) (string, error) {
	// LookPath also resolves npm's .cmd shims on Windows via PATHEXT
	if path, err := exec.LookPath(htmlServerName); err == nil {
		return path, nil
	}
	...
	if dir := npmGlobalBinDir(ctx); dir != "" {
		candidates := []string{htmlServerName}

		if runtime.GOOS == "windows" {
			candidates = []string{htmlServerName + ".cmd", htmlServerName + ".exe"}
		}
	...
	return "", fmt.Errorf("%v was not found in PATH or the npm global bin directory", htmlServerName)
}
...
// NormalizeDefinitionRange answers unchanged: the vscode html server rarely
// answers definitions at all, and what it answers spans the name.
func (hlsd *HTMLLanguageSupportDefinition) NormalizeDefinitionRange(_ *common.Source, r *common.FileRange) *common.FileRange {
	return r
}
