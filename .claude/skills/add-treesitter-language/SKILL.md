...
10. `NormalizeDefinitionRange()` — repair the shape of a `textDocument/definition` reply before anything resolves it to a node. Most servers answer with the declared name's extent; return the range untouched and you are done. sourcekit-lsp instead answers with a zero-width position at the name's start, and since ranges resolve by containment a range spanning no bytes matches nothing, so Swift widens it to the identifier. Probe your server against a fixture and look at the ranges it actually returns before deciding which case you are in. It runs inside `NewResolvedDependencyFromURI` with the target file's `*Source` in hand, so the buffer is available; put it in the LSP file next to `NewLSPServer`, never in the shared range queries.
